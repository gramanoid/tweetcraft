...
export interface SelectionResult {
  ...
  tabType?: "personas" | "all" | "smart" | "favorites" | "compose";
...
export class UnifiedSelector {
  ...
  hide(): void {
    ...
    if (this.container) {
      // Clean up menu close handler
      const menuHandler = (this.container as any)._menuCloseHandler;
      if (menuHandler) {
  ...
  private render(): void {
    ...
    this.container.innerHTML = `
      ...
      <div class="selector-header compact-toolbar">
        <div class="toolbar-left">
          <button class="quick-generate-btn primary-action" id="quickGenerateBtn" title="Quick Generate (Space)">
            <span class="quick-generate-icon">⚡</span>
            <span class="quick-generate-text">Generate</span>
          </button>
        </div>
        <div class="selector-tabs compact-tabs">
          <button class="tab-btn ${this.tabManager.getCurrentTab() === "personas" ? "active" : ""}" data-view="personas">
            👤 Personas
          </button>
          <button class="tab-btn ${this.tabManager.getCurrentTab() === "grid" ? "active" : ""}" data-view="grid">
            📝 All
          </button>
          <button class="tab-btn ${this.tabManager.getCurrentTab() === "smart" ? "active" : ""}" data-view="smart">
            🤖 Smart
          </button>
          <button class="tab-btn ${this.tabManager.getCurrentTab() === "favorites" ? "active" : ""}" data-view="favorites">
            ⭐ Favorites
          </button>
          <button class="tab-btn ${this.tabManager.getCurrentTab() === "compose" ? "active" : ""}" data-view="compose">
            ✍️ Compose
          </button>
          <button class="tab-btn ${this.tabManager.getCurrentTab() === "arsenal" ? "active" : ""}" data-view="arsenal">
            ⚔️ Arsenal
          </button>
          <button class="tab-btn ${this.tabManager.getCurrentTab() === "stats" ? "active" : ""}" data-view="stats">
            📊 Stats
          </button>
          <button class="tab-btn ${this.tabManager.getCurrentTab() === "trending" ? "active" : ""}" data-view="trending">
            🔥 Trending
          </button>
          <button class="tab-btn ${this.tabManager.getCurrentTab() === "engagement" ? "active" : ""}" data-view="engagement">
            📈 Engagement
          </button>
          <button class="tab-btn ${this.tabManager.getCurrentTab() === "abtest" ? "active" : ""}" data-view="abtest">
            🧪 A/B Test
          </button>
          <button class="tab-btn ${this.tabManager.getCurrentTab() === "cache" ? "active" : ""}" data-view="cache">
            💾 Cache
          </button>
        </div>
        <div class="toolbar-right">
          <button class="toolbar-menu-btn" title="More options">
            <span class="menu-icon">⋮</span>
          </button>
          <div class="toolbar-menu hidden">
            <button class="menu-item expanded-view-toggle" data-expanded="false">
              <span class="menu-icon">⊞</span>
              <span>Expanded View</span>
            </button>
            <button class="menu-item settings-btn">
              <span class="menu-icon">⚙️</span>
              <span>Settings</span>
            </button>
            <div class="menu-divider"></div>
            <button class="menu-item close-btn">
              <span class="menu-icon">×</span>
              <span>Close</span>
            </button>
          </div>
      ...
      <div class="selector-content ${this.tabManager.getCurrentTab()}-view" id="tabContent">
  ...
  private renderPersistentSelectionBar(): string {
    ...
    if (this.tabManager.getCurrentTab() === "personas" && this.selectedPersona) {
      selectionSummary = `
      ...
      `;
    } else if (this.tabManager.getCurrentTab() === "grid") {
      const items = [];
  ...
  private renderViewContent(): string {
    ...
    const tabType = tabMap[this.tabManager.getCurrentTab()] || "smart";
    
    // Schedule initial tab load and render
    // Force load even if it's the current tab to ensure component is loaded
    setTimeout(async () => {
      // Load the component if not already loaded
      const component = await this.tabManager.loadTabComponent(tabType as any);
      if (component) {
        // Render the tab content
        this.tabManager.renderCurrentTab();
      } else {
        // If still no component, try switching to force load
  ...
  private renderSelectionInfo(): string {
    if (this.tabManager.getCurrentTab() === "personas") {
  ...
  private attachEventListeners(): void {
    ...
    this.container.querySelectorAll(".tab-btn").forEach((btn) => {
      (btn as HTMLElement).addEventListener("click", (e) => {
        ...
        this.tabManager.switchTab(view).then(() => {
            this.render();
    ...
    // Toolbar menu button
    const menuBtn = this.container.querySelector(".toolbar-menu-btn");
    const menu = this.container.querySelector(".toolbar-menu");
    if (menuBtn && menu) {
      ...
      const closeMenuHandler = (e: MouseEvent) => {
        if (!menuBtn.contains(e.target as Node) && !menu.contains(e.target as Node)) {
          menu.classList.add("hidden");
        }
      };
      
      // Add to container for later cleanup
      setTimeout(() => {
        document.addEventListener("click", closeMenuHandler);
      }, 0);
      
      // Store handler for cleanup
      (this.container as any)._menuCloseHandler = closeMenuHandler;
    }
    
    // Settings button in menu
    const settingsBtn = this.container.querySelector(".menu-item.settings-btn");
    if (settingsBtn) {
    ...
    // Expanded view toggle in menu
    const expandedToggle = this.container.querySelector(".menu-item.expanded-view-toggle");
    if (expandedToggle) {
  ...
  private async applySmartDefaults(): Promise<void> {
    ...
    if (this.tabManager.getCurrentTab() === "grid") {
    ...
    } else if (this.tabManager.getCurrentTab() === "personas") {
