...
if platform.system() == "Windows":
    ...
    try:
        ...
        # Fallback: try Windows 10+ ANSI support
        try:
            import ctypes
...
def safe_emoji(emoji):
    """Safely output emoji, fallback to ASCII on Windows if needed."""
    if platform.system() == "Windows":
        try:
            emoji.encode('cp1252')
            return emoji
        except UnicodeEncodeError:
            # Fallback mappings for common emojis
            fallbacks = {
                "🔵": "[?]",
                "🚨": "[!!]",
                "🔴": "[!]",
            ...
            return fallbacks.get(emoji, "[?]")
    return emoji

def safe_chars(text):
    """Safely output Unicode characters, fallback on Windows if needed."""
    if platform.system() == "Windows":
        try:
            text.encode('cp1252')
            return text
        except UnicodeEncodeError:
            # Replace Unicode block characters with ASCII
...
def get_context_display(context_info):
    ...
    if percent >= 95:
        icon, color = safe_emoji("🚨"), "\033[31;1m"  # Blinking red
        alert = "CRIT"
    elif percent >= 90:
        icon, color = safe_emoji("🔴"), "\033[31m"    # Red
        alert = "HIGH"
    elif percent >= 75:
        icon, color = safe_emoji("🟠"), "\033[91m"   # Light red
        alert = ""
    elif percent >= 50:
        icon, color = safe_emoji("🟡"), "\033[33m"   # Yellow
        alert = ""
    else:
        icon, color = safe_emoji("🟢"), "\033[32m"   # Green
        alert = ""
    
    # Create progress bar
    segments = 8
    filled = int((percent / 100) * segments)
    bar = safe_chars("█" * filled + "▁" * (segments - filled))
...
def get_session_metrics(cost_data):
    ...
    if cost_usd > 0:
        ...
        metrics.append(f"{cost_color}{safe_emoji('💰')} {cost_str}\033[0m")
    ...
    if duration_ms > 0:
        ...
        metrics.append(f"{duration_color}{safe_emoji('⏱')} {duration_str}\033[0m")
    ...
    if lines_added > 0 or lines_removed > 0:
        ...
        metrics.append(f"{lines_color}{safe_emoji('📝')} {sign}{net_lines}\033[0m")
...
def main():
    try:
        ...
        status_line = f"{model_display} \033[93m{safe_emoji('📁')} {directory}\033[0m {safe_emoji('🧠')} {context_display}{session_metrics}"
        
        # Safe print with encoding fallback
        try:
            print(status_line)
        except UnicodeEncodeError:
            # Strip ANSI codes and use ASCII fallback
            import re
            clean_line = re.sub(r'\033\[[0-9;]*m', '', status_line)
            print(clean_line)
        
    except Exception as e:
        # Fallback display on any error
        print(f"\033[94m[Claude]\033[0m \033[93m{safe_emoji('📁')} {os.path.basename(os.getcwd())}\033[0m {safe_emoji('🧠')} \033[31m[Error: {str(e)[:20]}]\033[0m")
